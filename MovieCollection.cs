...
namespace TP3
{
    public class MovieCollection
    {
		...
		var Query1 = (from d in Releasedate 
                   select d).Min();
        

         //2 - Count all movies.
        int Query2 = Title.Count();

         //3 -Count all movies with the letter e. at least once in the title.
			int Query3 = Title.Count(f => f == 'e');

        //4- Count how many time the letter f is in all the titles from this list.

        var query4 = from t in Title

                     where t.Contains('f').count();

            //5-Display the title of the film with the higher budget.
        double highest_budget = (from b in budget
                    select b).Max();
        //6 -Display the title of the movie with the lowest box office.
        var Query6 = (from b in BoxOffice
                          select b).Min();
         //7 - Order the movies by reversed alphabetical order and print the first 11 of the list.
          var query7 = from w in WaltDisneyMovies
                       orderby w.title descending 

         //8 - Count all the movies made before 1980.
         var query8 = (from w in WaltDisneyMovies
                              where w.relaseDate =< 1980).count();


         //10 - Print all movies with the letter H or W in the title, but not the letter I or T.
          var query10 = from w in WaltDisneyMovies
                        where !w.title.Contains(" I ")
                        where !w.title.Contains(" T ")
                   
                        where w.title.ToLower().Contains("H" or "W")
